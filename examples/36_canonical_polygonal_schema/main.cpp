...
int main(int argc, char **argv)
{
    ...
    {
        std::cout << "\nCanonical Polygonal Schema" << std::endl;
        std::cout << "=========================" << std::endl;
        std::cout << "Usage: canonical_polygonal_schema [mesh_file] [root_vertex_id]" << std::endl;
        std::cout << "\nComputes a canonical polygonal schema (CPS) for surface meshing." << std::endl;
        std::cout << "If no mesh file is provided, defaults to: " << DATA_PATH << "/torus.obj" << std::endl;
        std::cout << "Optional root vertex ID (default: 152, best for torus)." << std::endl;
        std::cout << "\nDisplays both original mesh with cut loops and CPS parameterization." << std::endl;
        std::cout << "Controls: SHIFT+CLICK to reposition basis, rotate/pan/zoom with mouse." << std::endl;
        return 0;
    }
