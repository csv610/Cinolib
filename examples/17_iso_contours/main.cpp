#include <cinolib/meshes/meshes.h>
#include <cinolib/geodesics.h>
#include <cinolib/profiler.h>
#include <cinolib/drawable_isocontour.h>
#include <cinolib/gl/glcanvas.h>

int main(int argc, char **argv)
{
    using namespace cinolib;

    if(argc==2 && std::string(argv[1])=="-h")
    {
        std::cout << "\nIso Contours" << std::endl;
        std::cout << "============" << std::endl;
        std::cout << "Usage: iso_contours [mesh_file]" << std::endl;
        std::cout << "\nDisplays iso-contours on a surface mesh with a scalar field." << std::endl;
        std::cout << "If no mesh file is provided, defaults to: " << DATA_PATH << "/circle.obj" << std::endl;
        std::cout << "\nUse the slider to adjust the iso-value threshold." << std::endl;
        std::cout << "Controls: rotate (left click), pan (middle click), zoom (scroll)." << std::endl;
        return 0;
    }

    GLcanvas gui;

    std::string s = (argc==2) ? std::string(argv[1]) : std::string(DATA_PATH) + "/circle.obj";
    DrawableTrimesh<> m(s.c_str());
    compute_geodesics(m, {100}, COTANGENT).copy_to_mesh(m); // generate some scalar field
    m.show_texture1D(TEXTURE_1D_HSV_W_ISOLINES);
    m.show_wireframe(true);
    m.show_wireframe_transparency(0.5f);
    gui.push(&m);

    float iso_val = 0.5f;
    DrawableIsocontour<> iso(m, iso_val);
    iso.thickness = 3;
    gui.push(&iso, false);

    Profiler profiler;
    gui.callback_app_controls = [&]()
    {
        if(ImGui::SliderFloat("iso", &iso_val, 0.f, 1.f))
        {
            iso = DrawableIsocontour<>(m,iso_val);
            iso.thickness = 3;
        }
        if(ImGui::SmallButton("Tessellate"))
        {
            profiler.push("tessellate iso-contour");
            iso.tessellate(m);
            profiler.pop();
            m.updateGL();
        }
    };

    return gui.launch();
}
