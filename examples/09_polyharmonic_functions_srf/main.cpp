...
int main(int argc, char **argv)
{
    ...
    {
        std::cout << "\nPolyharmonic Functions (Surface)" << std::endl;
        std::cout << "===============================" << std::endl;
        std::cout << "Usage: polyharmonic_functions_srf [mesh_file]" << std::endl;
        std::cout << "\nInteractive harmonic function editor for surface meshes." << std::endl;
        std::cout << "Allows placing min/max boundary conditions by clicking on vertices." << std::endl;
        std::cout << "If no mesh file is provided, defaults to: " << DATA_PATH << "/bunny.obj" << std::endl;
        std::cout << "\nControls: SHIFT+CLICK to add maximum (red), CMD+CLICK to add minimum (blue)." << std::endl;
        std::cout << "Use GUI button to compute the harmonic field. Rotate/pan/zoom with mouse." << std::endl;
        return 0;
    }
