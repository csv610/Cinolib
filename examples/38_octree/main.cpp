...
int main(int argc, char **argv)
{
    ...
    if(argc==2 && std::string(argv[1])=="-h")
    {
        std::cout << "\nOctree" << std::endl;
        std::cout << "======" << std::endl;
        std::cout << "Usage: octree [mesh_file] [max_depth] [items_per_leaf]" << std::endl;
        std::cout << "\nBuilds and visualizes an octree for the mesh." << std::endl;
        std::cout << "If no mesh file is provided, defaults to: " << DATA_PATH << "/bunny.obj" << std::endl;
        std::cout << "Optional max_depth (default: 7), items_per_leaf (default: 50)." << std::endl;
        std::cout << "\nControls: SHIFT+CLICK to perform ray-trace intersection test." << std::endl;
        std::cout << "Use checkbox to toggle octree visualization." << std::endl;
